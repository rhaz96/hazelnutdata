...
From the thousands (if not millions) of share bags on the market (the population of m&m share bags), you obtained one sample of 50 bags.  You then meticulously count the number of m&m's in each bag, which inevitably differs from bag to bag.  Suppose the mean amount of m&m's in your sample turns out to be 29 and the standard deviation 5.  
...
I believe it's erroneous to teach the CLT the way Boston University's tutorial does.  My graduate stats class textbook^[[*The Statistical Sleuth*](http://statisticalsleuth.com/) (2013)] better illuminates the CLT.
...
* the shape of the distribution is approximately normal so long as the sample size is at least 30
...
However, the CLT **does not** require having multiple samples.  Rather, drawing multiple samples through simulation is a way to see that the CLT makes sense.  This is why I think the Boston University tutorial mentioned earlier is misleading.  
...
```{r}
z_score = (31-29)/(5/sqrt(50))
pnorm(2.828427, lower.tail = F)
```

### Conclusion

The main takeaway is the CLT enables us to derive the distribution of the sample mean from a just a single sample.  You can use simulation to show that *repeatedly* obtaining samples will produce a distribution of the sample mean like the one shown for m&m's above.  Repeated sampling has nothing to do with the CLT or its application. 
