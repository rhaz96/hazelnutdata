...
The definition implies that evaluating a linear function with some linear combination of inputs is the same as the linear combination of the inputs taken separately.

evaluating each of the inputs separately and summing them. 
...
Neither Savov nor the MIT page linked earlier show or derive the condition for when a generic line $y = mx+b$ can be considered a linear function.  It turns out that when $\beta$ = $1-\alpha$ it's indeed a linear function.
...
I was surprised to learn the generic equation of lines don't actually qualify as linear functions.  The only lines that do qualify to be linear functions are those that go through the origin; that is, their equation is $f(x) = mx$. So $b=0$ &mdash; you can verify this by going back to the proof above and subsituting $0$ for $b$. This ensures all values of $\alpha$ and $\beta$ meet the linear function requirements.  If $b$ $\neq$ $0$ then we must restrict $\beta$ to be equal to $1-\alpha$. That's just one of the gems found from studying linear algebra.  There's much more that you and I can uncover by learning this fascinating subject!
